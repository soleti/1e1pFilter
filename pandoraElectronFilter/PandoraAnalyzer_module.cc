...
class lee::PandoraAnalyzer : public art::EDAnalyzer {
public:
...
lee::PandoraAnalyzer::PandoraAnalyzer(fhicl::ParameterSet const & pset)
  :
  EDAnalyzer(pset)  // ,
...
lee::PandoraAnalyzer::~PandoraAnalyzer()
{
...
double lee::PandoraAnalyzer::distance(double a[3], double b[3]) {
  double d = 0;
...
art::Ptr<recob::Track> lee::PandoraAnalyzer::get_longest_track(std::vector< art::Ptr<recob::Track> > &tracks) {
...
void lee::PandoraAnalyzer::get_daughter_tracks(size_t ipf, const art::Event & evt, std::vector< art::Ptr<recob::Track> > &tracks) {
...
void lee::PandoraAnalyzer::get_daughter_showers(size_t ipf, const art::Event & evt, std::vector< art::Ptr<recob::Shower> > &showers) {
  art::InputTag pandoraNu_tag { "pandoraNu" };
...
double lee::PandoraAnalyzer::trackEnergy(const art::Ptr<recob::Track>& track, const art::Event & evt)
{
    art::InputTag pandoraNu_tag { "pandoraNu" };
...
void lee::PandoraAnalyzer::measure_energy(size_t ipf, const art::Event & evt, double & energy) {
...
size_t lee::PandoraAnalyzer::choose_candidate(std::vector<size_t> & candidates, const art::Event & evt) {
...
bool lee::PandoraAnalyzer::is_fiducial(double x[3]) const
{
...
bool lee::PandoraAnalyzer::is_dirt(double x[3]) const
{
  art::ServiceHandle<geo::Geometry> geo;
  double bnd[6] = {0.,2.*geo->DetHalfWidth(),-geo->DetHalfHeight(),geo->DetHalfHeight(),0.,geo->DetLength()};
...
void lee::PandoraAnalyzer::reconfigure(fhicl::ParameterSet const & pset)
{
...
DEFINE_ART_MODULE(lee::PandoraAnalyzer)
