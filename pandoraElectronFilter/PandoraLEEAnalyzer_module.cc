...
// std::vector< double > q lies on line segment 'pr'
bool lee::PandoraLEEAnalyzer::onSegment(std::vector< double > p, std::vector< double > q, std::vector< double > r)
{
...
// 1 --> Clockwise
// 2 --> Counterclockwise
int lee::PandoraLEEAnalyzer::orientation(std::vector< double > p, std::vector< double > q, std::vector< double > r)
{
...
// and 'p2q2' intersect.
bool lee::PandoraLEEAnalyzer::doIntersect(std::vector< double > p1, std::vector< double > q1, std::vector< double > p2, std::vector< double > q2)
{
...
// Returns true if the std::vector< double > p lies inside the polygon[] with n vertices
bool lee::PandoraLEEAnalyzer::isInside(std::vector< std::vector< double > > &polygon, std::vector< double > &p)
{
    ...
    do
    {
        ...
        {
            // If the std::vector< double > 'p' is colinear with line segment 'i-next',
...
void lee::PandoraLEEAnalyzer::dQdx(size_t pfp_id, const art::Event & evt, std::vector< double > &dqdx) {
  ...
  int correct = correct_direction(pfp_id, evt);
  std::cout << "[dQdx] *****" << "Correct direction? " << correct << std::endl;
  ...
  for (size_t icl = 0; icl < clusters.size(); icl++) {
    ...
    std::vector< double > cluster_axis = {cos(clusters[icl]->StartAngle()),sin(clusters[icl]->StartAngle())};
    ...
    // std::cout << "[dQdx] Point 0 " << points[0][0] << ", " << points[0][1] << std::endl;
    // std::cout << "[dQdx] Point 1 " << points[1][0] << ", " << points[1][1] << std::endl;
    // std::cout << "[dQdx] Point 2 " << points[2][0] << ", " << points[2][1] << std::endl;
    // std::cout << "[dQdx] Point 3 " << points[3][0] << ", " << points[3][1] << std::endl;
    // std::cout << "[dQdx] Axis " << cluster_axis[0] << ", " << cluster_axis[1] << std::endl;
    //
    // std::cout << "[dQdx] Cluster start " << cluster_start[0] << ", " << cluster_start[1] << std::endl;
    // std::cout << "[dQdx] Cluster end " << cluster_end[0] << ", " << cluster_end[1] << std::endl;
    ...
    for (auto & hit: hits) {
      ...
      // std::cout << "[dQdx] Hit " << hit_pos[0] << ", " << hit_pos[1];

      // Hit within the rectangle
      if (isInside(points, hit_pos) and pitch > 0) {
